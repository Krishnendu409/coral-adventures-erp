...
export async function updateBusinessParameter(param_id: string, value: number): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // Optional: add bounds checking using min_value and max_value here
    ...
    if (param.min_value !== null && value < param.min_value) return { success: false, error: `Value must be at least ${param.min_value}` };
    if (param.max_value !== null && value > param.max_value) return { success: false, error: `Value must be at most ${param.max_value}` };
